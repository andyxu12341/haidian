...
- 新增 4 张可编辑 SVG：数据证据分层、场地锚点审计、Space × Time 方法、定量 proof 结果。
- 当前 v0.4 仍为研究增强分支，不直接改变 #2093 正式 head；待正式 CI 稳定后再选择性并入。
...
- v0.3 完整二进制图件/PDF 已生成，但因 #2093 当时出现 manifest schema CI blocker，未直接整包推入正式 PR；研究分支先承载方法与数据深化。
